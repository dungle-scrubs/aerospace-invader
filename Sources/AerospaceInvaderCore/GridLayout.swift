...
public struct GridLayout {
    ...
    public init(itemCount: Int, itemSize: CGFloat, spacing: CGFloat, padding: CGFloat, headerHeight: CGFloat) {
    ...
    public var columns: Int { max(1, min(itemCount, GridLayout.maxColumns)) }

    /// Number of rows needed to hold every item.
    public var rows: Int { itemCount == 0 ? 0 : (itemCount + columns - 1) / columns }
    ...
    public func frame(forIndex index: Int, windowHeight: CGFloat) -> CGRect {
    ...
    public func index(forPoint point: CGPoint, windowHeight: CGFloat) -> Int {
